...
Claude Code has updated its model-related environment variables. `ANTHROPIC_SMALL_FAST_MODEL` is deprecated; new variables `ANTHROPIC_DEFAULT_HAIKU_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_OPUS_MODEL` are now the canonical way to override model selection. This epic updates ccctx to support the new variables while maintaining backward compatibility with existing config files and CLI flags.

### Story 4.1: Config & Runner Model Field Expansion

As a **developer**,
I want **config 和 runner 层扩展支持新的模型字段 (haiku_model, sonnet_model, opus_model)**,
So that **ccctx 能正确注入 Claude Code 当前的模型环境变量，用户可通过配置文件覆盖 haiku/sonnet/opus 模型选择**.

**Acceptance Criteria:**

**Given** config 中某个 context 配置了 `haiku_model = "claude-haiku-4-5-20251001"`
**When** `buildEnv()` 构建环境变量
**Then** 注入 `ANTHROPIC_DEFAULT_HAIKU_MODEL=claude-haiku-4-5-20251001`

**Given** config 中某个 context 同时配置了 `haiku_model` 和 `small_fast_model`
**When** `buildEnv()` 构建环境变量
**Then** `haiku_model` 优先，`small_fast_model` 作为 fallback

**Given** config 中某个 context 只配置了 `small_fast_model`（旧配置）
**When** `buildEnv()` 构建环境变量
**Then** 注入 `ANTHROPIC_DEFAULT_HAIKU_MODEL`（值为 small_fast_model 的值），不注入 `ANTHROPIC_SMALL_FAST_MODEL`

**Given** config 中某个 context 没有配置任何模型字段
**When** `buildEnv()` 构建环境变量
**Then** 不注入任何模型相关的 `ANTHROPIC_DEFAULT_*_MODEL` 变量

**Given** `runner.Options` 中的 `HaikuModel`、`SonnetModel`、`OpusModel` 字段
**When** 对应的 CLI flag 被使用
**Then** CLI flag 值优先于 config 文件值

**Files changed:** `config/config.go`, `internal/runner/runner.go`, `internal/runner/runner_test.go`, `config/config_test.go`

### Story 4.2: CLI Flag Extension & Backward Compatibility

As a **user**,
I want **通过 `--haiku-model`、`--sonnet-model`、`--opus-model` flags 覆盖模型选择**,
So that **我可以临时切换模型而不修改配置文件，且 `--small-fast-model` 仍作为 `--haiku-model` 的别名继续工作**.

**Acceptance Criteria:**

**Given** 运行 `ccctx run provider --haiku-model claude-haiku-4-5-20251001`
**When** ExtractFlags 解析参数
**Then** `haikuModel = "claude-haiku-4-5-20251001"`，不触发 TUI

**Given** 运行 `ccctx run provider --small-fast-model claude-haiku-4-5-20251001`
**When** ExtractFlags 解析参数
**Then** `haikuModel = "claude-haiku-4-5-20251001"`（与 `--haiku-model` 行为一致）

**Given** 同时使用 `--haiku-model` 和 `--small-fast-model`
**When** ExtractFlags 解析参数
**Then** `--haiku-model` 优先，`--small-fast-model` 被忽略

**Given** `ccctx list` 输出
**When** 运行
**Then** 输出不受影响

**Given** 无 flag 的 `ccctx run provider` 或 `ccctx exec provider`
**When** 运行
**Then** 行为与当前完全一致
...
so that **the interactive selection experience is robust, consistent, and diagnosable when things go wrong**._

> **Note:** Originally created during Epic 2 development from retrospective deferred items. Completed and moved to Epic 5 for tracking. Status: **done**.

**Acceptance Criteria:**

1. Terminal state restored when `app.Run()` returns error — `app.Stop()` called before returning
2. Flex width adapts to longest context name (min 30, max 80)
3. Magic numbers replaced with named constants (`minFlexWidth`, `maxFlexWidth`, `flexHeightPadding`)
4. Selection sources unified — both `SetDoneFunc` and `SetSelectedFunc` use `contexts[index]`
5. Cancellation message sent to stderr, not stdout
6. Panic recovery preserves stack trace via `runtime/debug.Stack()`
7. All tests pass, `go vet` clean, `make build` succeeds
8. Existing TUI behavior unchanged
